...
def draw_eval():
    global points, b
    end = [*[boundary for boundary in directory_boundaries if boundary > i], len(photo_paths)][0]
    start = [boundary for boundary in directory_boundaries[::-1] if boundary <= i][0]
    print(i, start, end, photo_paths[i].split('/')[-2])
    new_b = start
    if new_b != b:
        b = new_b
        if b in bs:
